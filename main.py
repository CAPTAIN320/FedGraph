...
sybils = [Sybil(k, graphs[-1], args) for k in range(args.num_sybils)]
for i in clients:
    i.g.remove_edges(i.g.edges(form='eid'))
    # i.g = add_self_loop(i.g)
    add_edges(i.g)
for s in sybils:
    s.g.remove_edges(s.g.edges(form='eid'))
    add_edges(s.g)
...
for _ in range(int(args.n_epochs)):
    ...
    # Merge
    all_clients = clients + sybils  # Combine regular clients and sybil clients
    server.merge(all_clients)
...
for s in sybils:
    acc = evaluate(s.model, s, mask='test')
    print("Sybil{}: {:.2%}".format(s.id, acc))
